...
def read_client_tcpdump(client_hostname:str, server_machine_name: str, thread_count: int, conn_count: int, rps:int, barrier):
    cmd = "python3 are-we-really-load-generating/new-experiments/common/tcpdumpreader.py " + server_machine_name
...
def run_server(server_machine_name: str, thread_count: int, conn_count: int, rps:int, experiment_name:str, barrier):
    ...
    for rps in rps_counts:
        for i, (conn, thread) in enumerate(configs):
            if conn >= thread:
                ...
                py_threads = []
                py_threads.append(threading.Thread(target=run_server, args=(server_machine_name, thread, conn, rps, experiment_name, barrier)))
