...
def install_server(server_machine_name:str, username):
    ...
    stdin, stdout, stderr = ssh_con.exec_command("git clone https://github.com/mittal1787/are-we-really-load-generating.git && cd are-we-really-load-generating && git pull origin main && cd new-experiments/experiment1 && sh install.sh")
    for i in range(4):
        try:
            stdin.write("Y\n")
            stdin.flush()
...
if __name__ == "__main__":
    ...
    if loadgen == "wrk2":
        ...
        experimentutils.run_wrk2_dsb(client_hostname, server_hostname, "experiment1", ssh_user=user)
    else:
        # Run all here
        experimentutils.install_wrk2(client_hostname, user)
        experimentutils.run_wrk2(client_hostname, server_hostname, "experiment1", ssh_user=user)
