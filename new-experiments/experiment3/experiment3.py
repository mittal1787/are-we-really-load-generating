...
def install_server(server_machine_name:str, username):
    ...
    stdin, stdout, stderr = ssh_con.exec_command("cd are-we-really-load-generating/new-experiments/experiment3 && sh install.sh")
    for i in range(4):
        try:
            stdin.write("Y\n")
            stdin.flush()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print('experiment3.py -c <client-hostname> -s <server-hostname> -g <load-generator> -u <username>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-c':
            client_hostname = arg
