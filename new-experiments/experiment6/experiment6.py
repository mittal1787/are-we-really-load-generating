...
def run_server_one(server_one: str, server_two:str, dir_name:str, barrier):
...
def run_server_two(server_two:str, dir_name:str, barrier):
    file_to_write = open(f"{dir_name}/server_arrival_times.csv","w")
    ssh_con = paramiko.SSHClient()
    ssh_con.load_system_host_keys()
    ssh_con.connect(server_two, username="yugm2")
...
def run_wrk2(client_hostname:str, server_one_hostname: str, server_two_hostname:str, experiment_name: str):
    ...
    for rps in rps_counts:
        for i, (conn, thread) in enumerate(configs):
            if conn >= thread:
                ...
                py_threads = []
                py_threads.append(threading.Thread(target=run_server_one, args=(server_one_hostname, server_two_hostname, dir_name, barrier)))
                py_threads.append(threading.Thread(target=run_server_two, args=(server_two_hostname, dir_name, barrier)))
                py_threads.append(threading.Thread(target=experimentutils.read_wrk_cpu_utilization, args=(client_hostname, dir_name, barrier)))
                py_threads.append(threading.Thread(target=experimentutils.run_wrk2_on_client_machine, args=(client_hostname, server_one_hostname, dir_name, barrier)))
                py_threads.append(threading.Thread(target=experimentutils.read_client_tcpdump, args=(client_hostname, server_one_hostname, client_to_s1_packets, barrier)))
                py_threads.append(threading.Thread(target=experimentutils.read_client_tcpdump, args=(server_one_hostname, server_two_hostname, s1_to_s2_packets, barrier)))
                py_threads.append(threading.Thread(target=experimentutils.read_server_cpu_utilization, args=(client_to_s1_packets, server_one_hostname, "serverone", barrier)))
                py_threads.append(threading.Thread(target=experimentutils.read_server_cpu_utilization, args=(s1_to_s2_packets, server_two_hostname, "servertwo", barrier)))
...
if __name__ == "__main__":
    ...
    if loadgen == "wrk2":
        ...
        run_wrk2(client_hostname, server_one_hostname, server_two_hostname, experiment_name="experiment6")
    else:
        # Run all here
        experimentutils.install_wrk2(client_hostname)
        run_wrk2(client_hostname, server_one_hostname, server_two_hostname, experiment_name="experiment6")
