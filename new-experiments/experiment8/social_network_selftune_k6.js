...
const url = "http://c220g2-010604.wisc.cloudlab.us:32291/wrk2-api";
...
//   const path = `http://localhost:8080/wrk2-api/user-timeline/read?${args}`;
  const path = `${url}/user-timeline/read?${args}`;
...
//   const path = `http://localhost:8080/wrk2-api/home-timeline/read?${args}`;
  const path = `${url}/home-timeline/read?${args}`;
  return http.get(path, {
    headers: headers
  })
}
...
function request() {
  ...
  const read_home_timeline_ratio = 0.45;
  const read_user_timeline_ratio = 0.45;
