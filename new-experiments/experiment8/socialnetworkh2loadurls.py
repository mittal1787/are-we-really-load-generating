...
# Initialize random seed
# random.seed(int(time.time() * 1000))
# for i in range(60*15*9000):
    # request()
os.makedirs("h2load_compose_body", exist_ok=True)
for i in range(1000):
    compose_post(f"h2load_compose_body/data_{i}.txt")
    with open(f'h2load_compose_body/data_{i}.txt', 'r') as f:
        content = f.read()
