...
def put_vegeta_compose_body_to_client(ssh_user:str, hostname:str):
    ...
    scp.put("new-experiments/experiment8/vegeta_socialnetwork_gp.txt", "vegeta_socialnetwork.txt")
...
def run_vegeta_on_client(ssh_user:str, master_hostname:str, other_clients:list, filename:str):
    ...
    stdin, stdout, stderr = ssh_con.exec_command("vegeta/vegeta attack -duration=15m -rate=10000 -targets=vegeta_socialnetwork.txt | vegeta/vegeta report -type=json")
    file_to_write = open(f"{filename}_vegeta_results.json","w")
    # print(stderr.read())
    file_to_write.write(stdout.read().decode("utf-8"))
    file_to_write.close()
    # stdin, stdout, stderr = ssh_con.exec_command("vegeta/vegeta attack -durati
    # on=15m -rate=1000 -targets=vegeta_socialnetwork.txt | tee results.bin | vegeta/vegeta report -type=json")
...
def run_h2load_on_clients(ssh_user:str, master_hostname:str, clients:list, server_machine_name:str, port:str, filename:str):
    ...
    ssh_con.connect(master_hostname, username=ssh_user)
    scp = SCPClient(ssh_con.get_transport())
    scp.put("new-experiments/experiment8/nghttp2_socialnetwork_new.txt", "nghttp2_socialnetwork.txt")
    ...
    for client in clients:
        for i in range(10):
            ...
            _, stdout_c, stderr = ssh_con_c.exec_command(f"nghttp2/src/h2load -t 1 -c 10 --rps=1 --h1 --duration={60*15} -d data_{data_num}.txt -H 'Content-Type: application/x-www-form-urlencoded' \"http://{server_machine_name}:{port}/wrk2-api/post/compose\"")
...
def run_locust_on_client_machines(ssh_user:str, master_hostname:str, clients:list, server_machine_name:str, port:str, filename:str):
    ...
    ssh_con_2.exec_command(f"locust -f socialnetworklocustfile.py --worker --processes 20")
...
def run_wrk2_dsb_on_client_machine(server_machine_name:str, port:str, lua_script_path, filename):
    # wrk = "./are-we-really-load-generating/new-experiments/DeathStarBench/wrk2/wrk"
    wrk = "new-experiments/DeathStarBench/wrk2/wrk"
    cmd = f"{wrk} -t2 -c100 -d15m -R10000 --requests --latency http://{server_machine_name}:{port}".split(" ")
    if lua_script_path != None:
        # cmd += " --script " + lua_script_path
        cmd.append("--script")
        cmd.append(lua_script_path)
    print("run_wrk2_on_client_machine: command = ", cmd)
    file_to_write = open(f"{filename}_wrk2_results.csv","w")
    wrk2_dsb_process = subprocess.Popen(cmd, stdout=subprocess.PIPE, text=True)
    output, error = wrk2_dsb_process.communicate()

    print("wrk2_dsb output = ", output)
    print("wrk2_dsb error", error)
...
def deploy_everything(ssh_user:str, server_machine_name:str, port:str, exp_num:int=0):
    ssh_con = paramiko.SSHClient()
    # ssh_con.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh_con.load_system_host_keys()
    ...
    if exp_num > 0:
        stdin, stdout, stderr = ssh_con.exec_command(f"cp are-we-really-load-generating/new-experiments/experiment8/recommender_deployment_{exp_num}.yaml autoscaler/vertical-pod-autoscaler/deploy/recommender-deployment.yaml")
        ssh_con.exec_command(f"cp are-we-really-load-generating/new-experiments/experiment8/config_{exp_num}.json DeathStarBench/socialNetwork/config/service-config.json")
    ...
    time.sleep(60)
    stdin, stdout, stderr = ssh_con.exec_command(f"cd DeathStarBench/socialNetwork && python3 scripts/init_social_graph.py --port={port}")
...
if __name__ == "__main__":
    ...
    server_hostname = "c220g2-011106.wisc.cloudlab.us"
    loadgen = None
    user = "yugm2"
    ...
    # put_vegeta_compose_body_to_client(user,client_hostname)
    ...
    for i in range(50):
        # kill_locust(user, client_hostname, other_clients)
        reset_everything(user, server_hostname)
        deploy_everything(user, server_hostname, port, i+1)
        ...
        run_wrk2_dsb_on_client_machine(server_hostname, port, lua_script_path="new-experiments/DeathStarBench/socialNetwork/wrk2/scripts/social-network/mixed-workload.lua", filename=f"new-experiments/experiment8/wrk2_dsb_results/iter{i+1}")
