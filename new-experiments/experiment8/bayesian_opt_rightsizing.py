...
parameters = [
        Integer(0, 24, name='cpu-histogram-decay-half-life'),
        Real(0.0,1.0,"uniform",name="recommendation-margin-fraction"),
        Integer(0,24000, name="pod-recommendation-min-cpu"),
        Integer(0,24, name="history-length"),
        # {
        #     "type": "discrete",
...
def get_reward(**params):
    ...
    """Negative RMSE Loss."""
    new_val = params["cpu-histogram-decay-half-life"]
    reward =  new_val - previous_value
        ...
        for service in service_data:
            if isinstance(service_data[service],dict):
                if "keepalive_ms" in service_data[service]:
                    parameters.append(Integer(0,10000,name=f"{service}_keepalive_ms"))
                    init_vals.append(service_data[service]["keepalive_ms"])
                    # parameters.append(
                    #     {
                    #         "type": "discrete",
                ...
                if "timeout_ms" in service_data[service]:
                    parameters.append(Integer(0,10000,name=f"{service}_timeout_ms"))
                    init_vals.append(service_data[service]["timeout_ms"])
                    # parameters.append(
                    #     {
                    #         "type": "discrete",
                ...
                if "connections" in service_data[service]:
                    parameters.append(Integer(0,512,name=f"{service}_connections"))
                    init_vals.append(service_data[service]["connections"])
                    # parameters.append(
                    #     {
                    #         "type": "discrete",
                ...
                if "use_cluster" in service_data[service]:
                    parameters.append(Integer(0,1,name=f"{service}_use_cluster"))
                    init_vals.append(service_data[service]["use_cluster"])
                    # parameters.append(
                    #     {
                    #         "type": "discrete",
