...
def deploy_everything(ssh_user:str, server_machine_name:str, exp_num:int=0):
    ...
    if exp_num > 0:
        stdin, stdout, stderr = ssh_con.exec_command(f"cp bayesianopt/recommender_deployment_{exp_num}.yaml autoscaler/vertical-pod-autoscaler/deploy/recommender-deployment.yaml")
        ssh_con.exec_command(f"cp bayesianopt/config_{exp_num}.json DeathStarBench/socialNetwork/config/service-config.json")
